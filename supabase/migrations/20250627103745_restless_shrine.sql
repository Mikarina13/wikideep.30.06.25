/*
  # Fixed Database Setup Migration

  1. New Tables
    - `users` - User profiles and metadata
    - `archive_posts` - AI-generated content posts  
    - `collab_posts` - Collaboration opportunities
    - `forum_posts` - Discussion posts and replies
    - `user_favorites` - User's favorited posts

  2. Security
    - Enable RLS on all tables
    - Add policies for proper access control
    - Create secure trigger functions
...
DO $$
BEGIN
  -- Add forum_posts_post_type_check constraint
  IF NOT EXISTS (
  ...
  -- Add user_favorites_post_type_check constraint
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
...
-- Try to create trigger on auth.users (if available)
DO $$
BEGIN
